...
## 🟢 Strong Matches
1. **Linux System Administration & Scripting**: Strong mastery of Linux environments, POSIX process management, file permissions, Shell/Bash scripting, and Python automation.
2. **Containerization**: Hands-on Docker experience containerizing cross-platform tools and microservice architectures.
3. **Cloud Services (AWS)**: Deep experience with AWS S3 object storage APIs, bucket policies, and AWS EC2 compute instances.
4. **CI/CD & Code Quality Verification**: Continuous integration discipline via GitHub Actions; ESLint core contribution passing **43 automated CI checks**.
5. **Distributed Systems & Queues**: Architecture of Redis & BullMQ job-processing systems with worker concurrency and distributed locking.
6. **Security & Cryptography**: Native alignment with CloudSEK's cybersecurity focus through verified implementation of **AES-256-GCM encryption**, key rotation, and JWT authentication.

---

## 🟡 Partial Matches
1. **Kubernetes**: Solid foundation in container orchestration principles, pod lifecycles, and declarative manifests; lacks multi-cluster production EKS operations.
2. **Traffic Routing & Ingress**: Experience with NGINX reverse proxy configuration and REST/HTTP routing; Gateway API (HTTPRoute) is conceptual.
...
2. **Observability Stack**: Prometheus, Grafana, and Loki monitoring pipelines not formally built in previous projects.
3. **GitOps Tooling**: ArgoCD has not been deployed in verified projects (GitHub Actions used instead).

---

## 📦 Projects Selected
1. **DB Backup CLI** (*TypeScript, Node.js, Redis, BullMQ, AWS S3, Docker*):
   * *Rationale:* Demonstrates pure DevOps and infrastructure tooling—Docker packaging, multi-database dump orchestration (PostgreSQL, MySQL, MongoDB, SQLite), Redis distributed worker queues, automated cron scheduling, and AWS S3 cloud offloading.
2. **PatentIQ | AI Prior-Art & RAG Engine** (*Fastify, Python, PostgreSQL, Pinecone, Docker*):
   * *Rationale:* Demonstrates backend system stability, high-dimensional vector search pipelines, Clean Architecture, and transactional persistence.
